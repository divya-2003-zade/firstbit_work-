...
// ADD PLAYERS 
void Addplayer(players *p, int *currentIndex)
{
    int n;
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        getchar();
        fgets(p[*currentIndex].name, sizeof(p[*currentIndex].name));
...
// SEARCH PLAYER BY JERSEY_NO AND NAME
